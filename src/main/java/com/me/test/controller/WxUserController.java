...
//        }
        ...
        //点击查看登录用户信息
    @GetMapping(value = "wx/query/person")
    @ResponseBody
    public String queryWxUserAndActivityByOpenID(@RequestParam(value = "open_id")String openid){
        System.out.println(openid);
        ...
        return JSON.toJSONString(wxUserList);
    }
    //编辑用户资料
    ...
    //查询编辑
    @PostMapping(value = "wx/query/person/{openid}")
    @ResponseBody
    public String queryWxUserByOpenID(@PathVariable("openid")String openid){
        System.out.println(openid);
        WxUser wxUser=wxUserService.queryWxUserById(openid);
        List<WxUser> wxUserList=new ArrayList<>();
        wxUserList.add(wxUser);
